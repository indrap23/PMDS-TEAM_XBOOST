...
def parse_input(data):
    ...
    if data["bureau"] == 'Yes':
        temp = temp.rename(columns = colname_bureau).drop(columns=["BUREAUDATA"])
        temp[["LOANS_WITHOUT_DELAYS","LOANS_WITH_DELAYS"]] = temp[["LOANS_WITHOUT_DELAYS","LOANS_WITH_DELAYS"]].astype(int)
    else :
        temp = temp.rename(columns = colname_bureau).drop(columns=["BUREAUDATA","LOANS_WITHOUT_DELAYS","LOANS_WITH_DELAYS"])
    temp[["AGE","INCOME"]] = temp[["AGE","INCOME"]].astype(int)
    return temp
